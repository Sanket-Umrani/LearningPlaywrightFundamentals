...
interface BuildSummary {
    runId: string;
    tests: Record<string, TestData['status']>;
}
...
type FlakyAgent = {
    analyzeFlaky: (prev: BuildSummary, curr: BuildSummary, useAi: boolean) => Promise<FlakyResult>;
};
...
const loadOptionalModule = <T>(modulePath: string): T | undefined => {
    try {
...
class CustomTTAReporter implements Reporter {
    ...
    private async runRcaAnalysis(): Promise<void> {
        ...
        const rcaAgent = loadOptionalModule<RcaAgent>('../ai/agents/rcaAgent');
        const providerConfig = loadOptionalModule<ProviderConfig>('../ai/config/providers');
        if (!rcaAgent || !providerConfig?.hasApiKey()) {
            console.log('🤖 RCA agent: unavailable or no LLM API key set — skipping AI verdict.');
            return;
        }
        ...
        for (const t of toAnalyze) {
            try {
                const verdict = await rcaAgent.analyzeFailure({
                    title: t.fullTitle,
                    file: t.location,
    ...
    private async runFlakyAnalysis(): Promise<void> {
        ...
        const flakyAgent = loadOptionalModule<FlakyAgent>('../ai/agents/flakyAnalyzer');
        const providerConfig = loadOptionalModule<ProviderConfig>('../ai/config/providers');
        if (!flakyAgent) {
            console.log('🔁 Flaky analyzer: unavailable — skipping comparison.');
            return;
        }

        this.flakyResult = await flakyAgent.analyzeFlaky(prev, curr, Boolean(providerConfig?.hasApiKey()));
